...
public class Mapping{
	...
	public Mapping()
	{
		...
		try
        {
        	Scanner sc = new Scanner(new File("knowledgebase/countries_id_map.txt"));
		...
		try
        {
        	Scanner sc = new Scanner(new File("knowledgebase/kb-facts-train_SI.tsv"));
	...
	public static void main(String[] args)
	{
		...
		Scanner sc = new Scanner(new File("sentences.tsv"));
		sc.useDelimiter("\n");
		...
		while (sc.hasNext())
		{
			...
			while(s1.hasNext())
			{
				...
				for(int i=0;i<a1.country.size();i++)
				{
					...
					{
						Double lower_range = a1.entities.get(j)-(a1.entities.get(j)*0.20);
						Double higher_range = a1.entities.get(j)+(a1.entities.get(j)*0.20);
